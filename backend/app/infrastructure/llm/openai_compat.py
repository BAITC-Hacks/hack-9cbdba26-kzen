...
Тебе дают: позицию, рекомендованное количество, срочность и только те компоненты
расчёта, которые действительно применены к этой позиции.
...
- если рекомендовано 0 шт, скажи, что заказ не требуется, и объясни это только
  переданными компонентами расчёта;
- упоминай исключение разовых продаж только при наличии соответствующей строки
  в компонентах расчёта, а восстановление спроса — только при наличии строки
  «Компенсация отсутствия товара»; если строка отсутствует, не упоминай эту
  поправку, даже если другая поправка есть;
- если такая строка есть, назови поправку явно для проверки менеджером;
...
def build_prompt(context: dict[str, Any]) -> str:
    ...
    lines.append("Применённые компоненты расчёта:")
    if reasons:
